...
const Busqueda = () => {
    ...
    const getEspecialidad = async () => {
        const res = await fetch(`http://localhost:3000/api/especialidad/${especialidad}`)
        ...
        <div className='h-screen'>
            ...
            <div className='grid grid-cols-2 m-6'>
                <div className='flex justify-end mr-4'>
                    <ComboboxEspceialidad especialidad={especialidad} setEspecialidad={setEspecialidad}/>
                </div>
                <div className='flex justify-start ml-4 max-h-10 w-64'>
                    <button className='w-full bg-indigo-500 text-white px-3 py-2 rounded hover:bg-indigo-600' onClick={getEspecialidad}>Buscar</button>
                </div>
            </div>
